...
def process_data(input_path, output_path):
    ...
    class FNN_test(nn.Module):
        ...
        def forward(self,x):
            ...
            out2[:,[2]]=-7 + (6.6)*out[:,[2]] 
            out2[:,[3]]=-0.699+0.6532*out[:,[3]]
            return out2
    FNN=FNN_test(input_size=4,hidden_size=156,output_size=4,layers=2)
    FNN.to(device=device)    

    model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),"VGM_H3_SWRC.pth")
    ...
    FNN.to(device=device)
    model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),"VGM_H3_K.pth")
    FNN.load_state_dict(torch.load(model_path))
    Tex_list=torch.tensor(np.array(Texture),dtype=torch.float64)  
    with torch.no_grad():  # Predict
        FNN.eval()
    ...
    pd.concat([pd.DataFrame(out_para_SWRC),pd.DataFrame(out_para_K)],axis=1).to_csv(output_path,header=['alpha','n','thr','ths','Ks']) # Save
