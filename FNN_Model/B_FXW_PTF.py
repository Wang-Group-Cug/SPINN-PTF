...
def process_data(input_path, output_path):
    ...
    class FNN_test(nn.Module):
        ...
        def forward(self,x):
            ...
            out2[:,[0]]=-2+2.17609125905568*out[:,[0]]
            out2[:,[1]]=-0.698970004336019+0.628388930050312*out[:,[1]]
            out2[:,[2]]=-3+2.3010*out[:,[2]] 
            out2[:,[3]]=0.0043+1.17176988527304*out[:,[3]] 
            out2[:,[4]] = 0 + 1.1761 * out[:, [4]] 
            return out2
    FNN=FNN_test(input_size=4,hidden_size=226,output_size=5,layers=3)
    FNN.to(device=device)    

    model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),"B_FXW_H3_SWRC.pth")
    ...
    FNN.to(device=device)
    model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),"B_FXW_H3_K.pth")
    FNN.load_state_dict(torch.load(model_path))
    Tex_list=torch.tensor(np.array(Texture),dtype=torch.float64)  
    with torch.no_grad():  # Predict
        FNN.eval()
    ...
    pd.concat([pd.DataFrame(out_para_SWRC),pd.DataFrame(out_para_K)],axis=1).to_csv(output_path,header=['m','ths','alpha','n','nc','Kha','Ks']) # Save
